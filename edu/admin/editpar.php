...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $ContactNumberNew = $_POST['ContactNumber'];
  $Address = $_POST['Address'];
  $ChildLRNs = $_POST['ChildLRN'] ?? [];


  $update = "UPDATE parents SET 
        FirstName = ?, MiddleName = ?, LastName = ?, Sex = ?, Birthdate = ?, ContactNumber = ?, Address = ? 
        WHERE parents_ID = ?";
  $stmt = $conn->prepare($update);
  $stmt->bind_param("sssssssi", $FirstName, $MiddleName, $LastName, $Sex, $Birthdate, $ContactNumberNew, $Address, $pid);
  $stmt->execute();
  ...
  foreach ($ChildLRNs as $lrn) {
    ...
    if ($res->num_rows > 0) {
      ...
      $checkLink->bind_param("ii", $pid, $studentID);
      $checkLink->execute();
      ...
      if ($exist->num_rows > 0) {
        ...
        $stmt2->bind_param("ii", $pid, $studentID);
        $stmt2->execute();
      }
    } else {
  ...
  if (empty($alertMessage)) {
    header("Location: parentinfo.php?search=" . urlencode($ContactNumberNew));
    exit();
  }
}
?>
      ...
      <div class="col-md-9 p-4">
        ...
        <?php elseif ($parent): ?>
          <div class="form-section">
            ...
            <form method="POST">
              <div class="row">
                <div class="col-md-4"><input type="text" name="FirstName" class="form-control" placeholder="First Name" required value="<?= htmlspecialchars($parent['FirstName']) ?>" /></div>
                <div class="col-md-4"><input type="text" name="MiddleName" class="form-control" placeholder="Middle Name" value="<?= htmlspecialchars($parent['MiddleName']) ?>" /></div>
                <div class="col-md-4"><input type="text" name="LastName" class="form-control" placeholder="Last Name" required value="<?= htmlspecialchars($parent['LastName']) ?>" /></div>
                <div class="col-md-4"><select name="Sex" class="form-control" required>
                    <option value="Male" <?= $parent['Sex'] == 'Male' ? 'selected' : '' ?>>Male</option>
                    <option value="Female" <?= $parent['Sex'] == 'Female' ? 'selected' : '' ?>>Female</option>
                  </select>
                </div>
                <div class="col-md-4"><input type="date" name="Birthdate" class="form-control" required value="<?= htmlspecialchars($parent['Birthdate']) ?>" /></div>
                <div class="col-md-4"><input type="text" name="ContactNumber" class="form-control" maxlength="11" pattern="\d{11}" required placeholder="Contact Number" value="<?= htmlspecialchars($parent['ContactNumber']) ?>" /></div>
                <div class="col-md-12"><input type="text" name="Address" class="form-control" placeholder="Address" required value="<?= htmlspecialchars($parent['Address']) ?>" /></div>
                    ...
                    <?php if ($childrenResult->num_rows > 0): ?>
                      <?php while ($child = $childrenResult->fetch_assoc()): ?>
                        <li class="list-group-item">
                          <?= htmlspecialchars($child['LRN']) ?> - <?= htmlspecialchars($child['FirstName'] . " " . $child['MiddleName'] . " " . $child['LastName']) ?> (<?= htmlspecialchars($child['Status']) ?>)
                        </li>
                      <?php endwhile; ?>
                    <?php else: ?>
                ...
                <div class="col-12 text-center mt-4">
                  ...
                  <a href="parentinfo.php" class="btn btn-secondary ms-2">CANCEL</a>
                </div>
              </div>
            </form>
          </div>
